...
def evalu(Y_true,Y_pred,name):
    for i in range(21):
        ...
        c=["#264653",'#287271','#2A9D8C','#8AB07D','#E9C46B','#F3A261','#E66F51']
        plt.plot(Y_true[:, i], Y_pred[:, i], '.',color=c[3])
        plt.plot(Y_true[:, i], Y_true[:, i], '-',color=c[5])
        plt.xlabel('True')
...
#%%
#模型搭建及数据读取
    m = nn.Sequential(nn.Linear(119, layer1), nn.Tanh(), nn.Linear(layer1, layer2), nn.Tanh(), nn.Linear(layer2, layer3), nn.ReLU(),nn.Linear(layer3,21))
    m.double()   
...
##训练开始、评估   
    m,loss_hist,R2_hist= train(m, X_train, Y_train, max_iter=50000)
